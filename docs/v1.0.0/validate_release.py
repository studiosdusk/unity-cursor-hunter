"""Release design checks: arithmetic, dependency graph, screen routes and PDF links."""
...
pdf=ROOT/'output/pdf/cursor-hunter-game-design-v1.0.0.pdf';r=PdfReader(pdf)
...
assert '\ufffd' not in text
report=dict(status='passed',pages=len(r.pages),screen_states=len(screens),screen_routes_reachable=len(visited),pdf_links=links,trait_nodes=len(nodes),purchasable_nodes=114,partial_upgrade_checks=checks,late_upgrade_wait=late,source_sha256=hashlib.sha256((ROOT/'docs/cursor-hunter-game-design-v1.0.0.md').read_bytes()).hexdigest(),pdf_sha256=hashlib.sha256(pdf.read_bytes()).hexdigest(),limitations=['Not a Unity build or gameplay simulation','2-hour completion and 60% income yield remain playtest targets','Visual review reported separately'])
(ROOT/'output/pdf/cursor-hunter-v1.0.0-validation.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n')
