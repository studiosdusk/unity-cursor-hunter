...
SOURCE=ROOT/'docs/cursor-hunter-game-design-v0.0.4.md'
...
def assemble_source():
    """Assemble a design-only v0.0.4 document.

    Release review, external references, and implementation status are kept
    outside the PDF. The PDF contains the plan core, numeric rule books, and
    the 48 screen/state specifications only.
    """
    def clean(text):
        replacements={
            '### 자동 스킬 초안':'### 자동 스킬',
            ...
            '이번 초안':'설계안',
            '초안 상한':'표시 상한',
            'A04를 우선 적용한다':'부분 강화 규칙을 적용한다',
            '추천은 동일한 클릭 판정을 유지하며 자동10→30회로 성장시키는 방식이다. 즉시30회는 구현이 단순하지만 성장 단계가 하나 사라지고 부하가 급증한다. 단계형은 속도 레벨 관리가 추가되지만 밸런스·성능 측정과 후반 확장에 유리하다.':'자동 클릭은 10→15→20→25→30묶음/초 단계로 성장한다.',
            '검산':'계산',
            'PDF의 계속 버튼은 일반 대표 경로다.':'계속은 진입한 Field와 남은 시간을 복원한다.',
            'S07은 수동 표시 참고만.':'',
            '수치는 예시.':'수치는 해당 진행 상태의 표시 값이다.',
            '첫 검증에서 제외한다.':'이 설계에 포함하지 않는다.',
            '### 패턴 초안':'### 패턴',
            '추천 레벨':'기준 레벨',
            '예시는보너스10%.':'보너스는 파훼 횟수에 따라 계산한다.',
            '기획상 동작이며실제파일초기화를이번작업에서수행하지않는다.':'초기화 후 새 저장을 만들고 기존 저장은 백업으로 보존한다.',
            'PDF에서END는실제프로세스를종료하지않는다.':'저장 후 프로세스를 종료한다.',
            '예시성공버튼은PDF검토용이며실제UI에는없음.':'저장 완료 뒤 다음 화면 이동 버튼을 활성화한다.',
            '최저사양수치는실측전미확정.':'지원 해상도는 표시 장치가 제공하는 값으로 구성한다.',
            '출시기획표참조.':'도전과제 조건 표에 따른다.',
            '참고만':'',
        }
    ...
    chunks=[(HERE/'plan_core.md').read_text().strip()]
    source=(ROOT/'docs/cursor-hunter-game-design-v1.0.0.md').read_text()
    for part in source.split('<!-- pagebreak -->'):
        heading=next((line for line in part.splitlines() if line.startswith('## B0')),None)
        if heading:
            chunks.append(clean(part))
...
'''
    for i in range(16):
        flow+='| '+' | '.join(f'[{SCREENS[k]["id"]} {SCREENS[k]["title"].split(" · ")[0]}](#{SCREENS[k]["id"]})' for k in [i,i+16,i+32])+' |\n'
    chunks.append(flow.strip())

    for screen in SCREENS:
        actions=' / '.join(f"{a['label']} → {a['target']}" for a in screen['actions'])
        chunks.append(clean(f'''## {screen['id']}. {screen['title']}
...
{screen['entry']}. 이 화면은 {screen['title']} 상태를 다룬다.
...
class ReleaseWire(Flowable):
    ...
    def draw(self):
        ...
        c.setFont('Korean',6.5);c.setFillColor(colors.HexColor('#53647A'));c.drawString(0,16*mm,'화면 이동 (자동 전이와 대표 복귀 경로)')
    ...
    def visible_gem_count(self):
        # Only gems unlocked by the current progress state are rendered.
        return {'S05':1,'S06':1,'S07':1,'S11':1,'S12':1,'S13':1,'S14':1,'S15':1,
                'S16':2,'S17':3,'S18':2,'S19':2,
                'S20':2,'S21':2,'S22':2,'S23':2,'S24':2,'S25':2,'S26':2,'S27':2,'S30':2,
                'S28':3,'S29':3,'S08':5}.get(self.s['id'],1)
    def gems(self,left=False):
        names=['가넷','토파즈','자수정','사파이어','다이아'];cs=['#C75C61','#E7B747','#9D70CE','#569ADC','#E0EAF7']
        count=self.visible_gem_count()
        amounts=(['0','0','0','0','0'] if self.s['id'] in ['S05','S06'] else
                 ['96','0','0','0','0'] if self.s['id'] in ['S07','S11'] else
                 ['45,000','60','30','15','0'] if self.s['id']=='S19' else
                 ['50,000','10','30','15','0'] if self.s['id']=='S18' else
                 ['50,000','85','30','15','0'])
        if left:
            self.text(24,142,'잼 스톤 보유',16)
            for i,(name,col) in enumerate(zip(names[:count],cs[:count])):
                y=177+i*48;self.rect(20,y,192,38,'#223852','#516682');self.rect(30,y+9,18,18,col);self.text(56,y+10,name,13);self.text(126,y+10,amounts[i],13)
        else:
            start=985-count*96
            for i,(name,col) in enumerate(zip(names[:count],cs[:count])):
                x=start+i*96;self.rect(x,17,90,56,'#263C50');self.text(x+9,23,name,12,col);self.text(x+9,43,amounts[i],16)
    ...
    def main_screen(self,k):
        self.landscape();self.text(335,121,'CURSOR HUNTER',38,'#213A31')
    ...
    def selection(self,k):
        ...
        self.text(701,145,'보상 / 첫 처치 해금',21);self.block(701,196,'가넷50,000\n잠긴 보상\n자동 / 골렘 / 프리징\nHP180,000' if k=='boss-locked' else '가넷2,000 / 토파즈35\n잠긴 보상\n트리플 / 오크 / 라이트닝\n현재 이론DPS400',246,18)
        self.footer_buttons()
    def tree(self,k):
    ...
    def summary(self):
        self.gems(True);self.text(28,27,'현재 특성 / 보유 효과 전체',28)
    ...
    def settings(self,k):
        ...
        self.footer_buttons()
    def result(self,k):
        self.gems();self.text(32,27,'< Main',18);self.text(306,38,'보스 도전 실패' if k=='failure' else '보스 격파' if k=='boss-result' else '일반 Field 완료',31,self.gold)
        ...
        gem_label=' · '.join(['가넷','토파즈','자수정','사파이어','다이아'][:self.visible_gem_count()])
...
def footer(c,d):
    c.saveState();w,h=A4;c.setFillColor(colors.HexColor('#172A43'));c.rect(0,h-9*mm,w,9*mm,fill=1,stroke=0);c.setFont('Korean',8);c.setFillColor(colors.white);c.drawString(18*mm,h-6*mm,'CURSOR HUNTER / v0.0.4 DESIGN')
    c.setFillColor(colors.HexColor('#66758A'));c.drawString(18*mm,9*mm,'커서 액션 성장 게임 설계');c.drawRightString(w-18*mm,9*mm,str(c.getPageNumber()));c.restoreState()

def main():
    generate_traits();assemble_source();base.register_fonts();OUTPUT.parent.mkdir(parents=True,exist_ok=True)
    doc=BaseDocTemplate(str(OUTPUT),pagesize=A4,leftMargin=18*mm,rightMargin=18*mm,topMargin=17*mm,bottomMargin=18*mm,title='Cursor Hunter v0.0.4 세부 기획·전체 화면',author='Codex')
